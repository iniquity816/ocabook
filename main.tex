...
\noindent \textsc{\href{https://iniquity816.github.io/ocabook/}{https://iniquity816.github.io/ocabook/}}\\ % URL

\noindent \textit{The Destruction of The World May 15th 2026, 44 Days After The Rapture} % Printing/edition date
...
\begin{itemize}
	...
	\item $2^{15}$ ( = 32768) of them have the lowest birth condition as two neighbours, meaning lightspeed frontends made of dominos can propagate through the universe and subsequently cause explosions.
	...
	\item $2^{14}$ of them have the lowest birth condition as four or more neighbours, meaning growth is impossible and patterns often localize in boxes that are unable to grow or expand by themselves.
	\item $2^{17}$ (= 131072) of them have the lowest birth condition as zero neighbours. If the highest survival condition in a rule in this family is eight neighbours (the maximum), then no finite patterns can exist, as the background becomes filled with infinitely many alive cells. If the highest survival condition is not eight neighbours, as is the case in $2^{16}$ of them, then the background flickers between dead and alive each generation; this is often circumvented by CA simulation software in order to avoid simulating infinite cells.
\end{itemize} 
...
\chapterimage{ot.png} % Chapter heading image
\chapterspaceabove{6.75cm} % Whitespace from the top of the page to the chapter title on chapter pages
\chapterspacebelow{7.25cm} % Amount of vertical whitespace from the top margin to the start of the text on chapter pages
